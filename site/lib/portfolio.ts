...
const followupsByIntent: Record<PortfolioIntent, string[]> = {
  ...
  unknown: ['What is your experience?', 'Show me your main projects.', 'How can I contact you?'],
};
...
const intentMatchers: Array<{ intent: PortfolioIntent; patterns: RegExp[] }> = [
  ...
  {
    intent: 'contact',
    patterns: [/\bcontact\b/i, /\bemail\b/i, /\breach\b/i, /\blinkedin\b/i, /\bgithub\b/i, /\bhire\b/i],
  },
  {
    intent: 'availability',
...
export const generateFallbackAnswer = (intent: PortfolioIntent, message: string) => {
  switch (intent) {
    ...
    default:
      return `I can help with Sabith’s experience, projects, skills, resume, or contact details. Try asking about his frontend background, recent products, or the kind of work he has shipped.`;
  }
};
...
export const buildIntentContext = (intent: PortfolioIntent, message: string) => {
  ...
  if (intent === 'about' || intent === 'experience') {
  ...
  if (intent === 'resume' || intent === 'contact' || intent === 'availability') {
    sections.push(
